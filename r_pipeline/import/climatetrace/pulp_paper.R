# =============================================================================
# FILE: r_pipeline/import/climatetrace/pulp_paper.R
# Import and process Climate Trace pulp_paper (pulp-and-paper) data
# Migrated from CarbonBudgetTracker/02.Scripts/01_import/climatetrace/pulp_paper.R
# =============================================================================
...
cat(">> Importing Climate Trace Pulp & Paper Data...\n\n")
...
ct_pulp_paper_emissions <- file.path(ct_manufacturing_path,
                                    paste0("pulp-and-paper_emissions_sources_", ct_version, ".csv"))
ct_pulp_paper_ownership <- file.path(ct_manufacturing_path,
                                    paste0("pulp-and-paper_emissions_sources_ownership_", ct_version, ".csv"))

if (!file.exists(ct_pulp_paper_emissions)) stop("Pulp & Paper emissions file not found: ", ct_pulp_paper_emissions)
if (!file.exists(ct_pulp_paper_ownership)) stop("Pulp & Paper ownership file not found: ", ct_pulp_paper_ownership)

cat(">> Loading files:\n")
cat("   Emissions:", basename(ct_pulp_paper_emissions), "\n")
cat("   Ownership:", basename(ct_pulp_paper_ownership), "\n\n")
...
emissions_raw <- read_csv(ct_pulp_paper_emissions, show_col_types = FALSE)
cat("   Loaded", nrow(emissions_raw), "emission records\n")

cat(">> Reading ownership data...\n")
ownership_raw <- read_csv(ct_pulp_paper_ownership, show_col_types = FALSE)
...
export_csv(company_annual,         "climatetrace_pulp_paper_company_annual.csv",
           dir = outputs_fossil_dir)
export_csv(company_country_annual, "climatetrace_pulp_paper_company_country_annual.csv",
           dir = outputs_fossil_dir)
export_csv(facility_annual,        "climatetrace_pulp_paper_facility_annual.csv",
           dir = outputs_fossil_dir)

cat("\n>> Climate Trace pulp & paper import complete!\n")
