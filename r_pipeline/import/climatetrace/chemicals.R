# =============================================================================
# FILE: r_pipeline/import/climatetrace/chemicals.R
# Import and process Climate Trace chemicals (chemicals) data
# Migrated from CarbonBudgetTracker/02.Scripts/01_import/climatetrace/chemicals.R
# =============================================================================
...
cat(">> Importing Climate Trace Chemicals Data...\n\n")
...
ct_chemicals_emissions <- file.path(ct_manufacturing_path,
                                   paste0("chemicals_emissions_sources_", ct_version, ".csv"))
ct_chemicals_ownership <- file.path(ct_manufacturing_path,
                                   paste0("chemicals_emissions_sources_ownership_", ct_version, ".csv"))

if (!file.exists(ct_chemicals_emissions)) stop("Chemicals emissions file not found: ", ct_chemicals_emissions)
if (!file.exists(ct_chemicals_ownership)) stop("Chemicals ownership file not found: ", ct_chemicals_ownership)

cat(">> Loading files:\n")
cat("   Emissions:", basename(ct_chemicals_emissions), "\n")
cat("   Ownership:", basename(ct_chemicals_ownership), "\n\n")
...
emissions_raw <- read_csv(ct_chemicals_emissions, show_col_types = FALSE)
cat("   Loaded", nrow(emissions_raw), "emission records\n")

cat(">> Reading ownership data...\n")
ownership_raw <- read_csv(ct_chemicals_ownership, show_col_types = FALSE)
...
export_csv(company_annual,         "climatetrace_chemicals_company_annual.csv",
           dir = outputs_fossil_dir)
export_csv(company_country_annual, "climatetrace_chemicals_company_country_annual.csv",
           dir = outputs_fossil_dir)
export_csv(facility_annual,        "climatetrace_chemicals_facility_annual.csv",
           dir = outputs_fossil_dir)

cat("\n>> Climate Trace chemicals import complete!\n")
