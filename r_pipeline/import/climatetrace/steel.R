# =============================================================================
# FILE: r_pipeline/import/climatetrace/steel.R
# Import and process Climate Trace steel (iron-and-steel) data
# Migrated from CarbonBudgetTracker/02.Scripts/01_import/climatetrace/steel.R
# =============================================================================
...
cat(">> Importing Climate Trace Steel Data...\n\n")
...
ct_steel_emissions <- file.path(ct_manufacturing_path,
                                paste0("iron-and-steel_emissions_sources_", ct_version, ".csv"))
ct_steel_ownership <- file.path(ct_manufacturing_path,
                                paste0("iron-and-steel_emissions_sources_ownership_", ct_version, ".csv"))

if (!file.exists(ct_steel_emissions)) stop("Steel emissions file not found: ", ct_steel_emissions)
if (!file.exists(ct_steel_ownership)) stop("Steel ownership file not found: ", ct_steel_ownership)

cat(">> Loading files:\n")
cat("   Emissions:", basename(ct_steel_emissions), "\n")
cat("   Ownership:", basename(ct_steel_ownership), "\n\n")
...
emissions_raw <- read_csv(ct_steel_emissions, show_col_types = FALSE)
cat("   Loaded", nrow(emissions_raw), "emission records\n")

cat(">> Reading ownership data...\n")
ownership_raw <- read_csv(ct_steel_ownership, show_col_types = FALSE)
...
export_csv(company_annual,         "climatetrace_steel_company_annual.csv",
           dir = outputs_steel_dir)
export_csv(company_country_annual, "climatetrace_steel_company_country_annual.csv",
           dir = outputs_steel_dir)
export_csv(facility_annual,        "climatetrace_steel_facility_annual.csv",
           dir = outputs_steel_dir)

cat("\n>> Climate Trace steel import complete!\n")
