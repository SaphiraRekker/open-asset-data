# =============================================================================
# FILE: r_pipeline/import/climatetrace/petrochemical.R
# Import and process Climate Trace petrochemical (petrochemical-steam-cracking) data
# Migrated from CarbonBudgetTracker/02.Scripts/01_import/climatetrace/petrochemical.R
# =============================================================================
...
cat(">> Importing Climate Trace Petrochemical Data...\n\n")
...
ct_petrochemical_emissions <- file.path(ct_manufacturing_path,
                                       paste0("petrochemical-steam-cracking_emissions_sources_", ct_version, ".csv"))
ct_petrochemical_ownership <- file.path(ct_manufacturing_path,
                                       paste0("petrochemical-steam-cracking_emissions_sources_ownership_", ct_version, ".csv"))

if (!file.exists(ct_petrochemical_emissions)) stop("Petrochemical emissions file not found: ", ct_petrochemical_emissions)
if (!file.exists(ct_petrochemical_ownership)) stop("Petrochemical ownership file not found: ", ct_petrochemical_ownership)

cat(">> Loading files:\n")
cat("   Emissions:", basename(ct_petrochemical_emissions), "\n")
cat("   Ownership:", basename(ct_petrochemical_ownership), "\n\n")
...
emissions_raw <- read_csv(ct_petrochemical_emissions, show_col_types = FALSE)
cat("   Loaded", nrow(emissions_raw), "emission records\n")

cat(">> Reading ownership data...\n")
ownership_raw <- read_csv(ct_petrochemical_ownership, show_col_types = FALSE)
...
export_csv(company_annual,         "climatetrace_petrochemical_company_annual.csv",
           dir = outputs_fossil_dir)
export_csv(company_country_annual, "climatetrace_petrochemical_company_country_annual.csv",
           dir = outputs_fossil_dir)
export_csv(facility_annual,        "climatetrace_petrochemical_facility_annual.csv",
           dir = outputs_fossil_dir)

cat("\n>> Climate Trace petrochemical import complete!\n")
