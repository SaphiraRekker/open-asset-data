# =============================================================================
# FILE: r_pipeline/import/climatetrace/aluminum.R
# Import and process Climate Trace aluminum (aluminum) data
# Migrated from CarbonBudgetTracker/02.Scripts/01_import/climatetrace/aluminum.R
# =============================================================================
...
cat(">> Importing Climate Trace Aluminum Data...\n\n")
...
ct_aluminum_emissions <- file.path(ct_manufacturing_path,
                                  paste0("aluminum_emissions_sources_", ct_version, ".csv"))
ct_aluminum_ownership <- file.path(ct_manufacturing_path,
                                  paste0("aluminum_emissions_sources_ownership_", ct_version, ".csv"))

if (!file.exists(ct_aluminum_emissions)) stop("Aluminum emissions file not found: ", ct_aluminum_emissions)
if (!file.exists(ct_aluminum_ownership)) stop("Aluminum ownership file not found: ", ct_aluminum_ownership)

cat(">> Loading files:\n")
cat("   Emissions:", basename(ct_aluminum_emissions), "\n")
cat("   Ownership:", basename(ct_aluminum_ownership), "\n\n")
...
emissions_raw <- read_csv(ct_aluminum_emissions, show_col_types = FALSE)
cat("   Loaded", nrow(emissions_raw), "emission records\n")

cat(">> Reading ownership data...\n")
ownership_raw <- read_csv(ct_aluminum_ownership, show_col_types = FALSE)
...
export_csv(company_annual,         "climatetrace_aluminum_company_annual.csv",
           dir = outputs_aluminium_dir)
export_csv(company_country_annual, "climatetrace_aluminum_company_country_annual.csv",
           dir = outputs_aluminium_dir)
export_csv(facility_annual,        "climatetrace_aluminum_facility_annual.csv",
           dir = outputs_aluminium_dir)

cat("\n>> Climate Trace aluminum import complete!\n")
